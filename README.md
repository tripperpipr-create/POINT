...
**Цель продукта:** [управляемый цикл разработки в Point IDE](docs/PRODUCT-VISION.md). [Текущее состояние и открытые проверки](docs/PROJECT-STATUS.md) отделены от целевой архитектуры.
...
Текущая версия Point — `1.2.3`, Code-OSS — `1.124.2`. Agent Hub использует
...
VSIX создаётся в `build/vsix/point-ide-1.2.3.vsix`. Production-дистрибутив уже
...
WorkOrder v2 и быстрый Agent v2 работают в отдельной копии или detached Git
worktree; другие маршруты могут иметь явно выбранный live-write режим.
Пользователь видит выбранный режим до запуска. Patch требует
