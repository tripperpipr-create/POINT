...
| 10 | GitLab и MCP | Гильдия → «Интеграции»: плагин GitLab (адрес, личный токен, CA), свои MCP-серверы формой или импортом `mcp.json`, разовое доверие к команде, секреты в SecretStorage, инструменты с отпечатком и риском. Окно GitLab: MR «Мои / На ревью / Все открытые» и пайплайны ветки проекта папки; карточка MR с обсуждением, diff IDE, одобрением и merge по подтверждению | `internal/integrations/gitlab/client_test.go`, `internal/app/gitlab_test.go`, `internal/app/mcp_servers_test.go`, `internal/mcpclient/*_test.go`, `smoke-mcp-integrations.js`, `smoke-gitlab-tool-window.js` | Исходники и фикстуры готовы; живая приёмка на GitLab 19.3.2 владельца не пройдена |
