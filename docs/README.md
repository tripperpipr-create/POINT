...
- **Зонды по живому приложению.** Требуют запущенного Point с CDP-эндпоинтом и
  ...
  `test-point-*.ps1`, `run-hub-dogfood.mjs` и `run-live-loop.mjs`, которым нужна
  живая модель.

  `run-live-loop.mjs` — полигон повседневной доводки (`make loop`): лёгкая
  правка в настоящем проекте от реплики до зелёных тестов, круг полминуты. Без
  ключа он проверяет только правила вердикта и выходит с нулём, поэтому в
  затвор его звать бессмысленно, а гонять руками — стоит: на нём видно то, чего
  не видит ни один тест, — сколько шагов агент тратит впустую и что человек
  читает вместо объяснения.
...
Возврат из веток кликов стережёт `scripts/check-click-handler-returns.mjs`.
Цепочка обработчиков останавливается на первом, вернувшем `true`, и ветка,
вышедшая голым `return`, пускает клик дальше по всем остальным. Так в дереве
накопилось двадцать семь таких веток, восемнадцать — с недостижимым
`return true` строкой ниже: намерение записано, работает соседний оператор.
Ни `node --check`, ни сверка имён такого не видят, потому что синтаксис цел и
все имена на месте. Затвор разбирает только блочные ветки внутри тел
`handle*Action` и держит сторож «находок мало»: проверка, переставшая что-либо
видеть после переезда модулей, проходит вхолостую и молчит об этом громче
всего.
