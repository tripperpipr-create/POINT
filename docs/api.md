...
Master quest controls operate on the execution runtime, not only the quest row. `pause` first closes Flow scheduling and requests a safe checkpoint from every active headless run; an active interactive CLI that cannot pause is reported instead of pretending success. `resume` reopens the Flow, restores resumable executions and schedules the next nodes. Its optional `apiKey` is transient SecretStorage material and is never added to the immutable control journal. `resume` is refused once the evidence gate has reached a verdict for the quest: a fix is a new work order version, which runs as a new quest. A quest whose Flow finished without a verdict resumes into the verification it missed, and a quest paused by the core (Docker unavailable, repair attempt after failed host checks) carries `runtime.waitingForSandbox` or `runtime.resumeAfterRestart`, so the extension continues it by itself. `cancel` terminates active headless/CLI executions and marks pending executions and the Flow terminal. `message` is secret-redacted before it is injected into active runs and before it is stored in the immutable coordinator journal. Responses include `flowRunId` and `affectedRuns`.
