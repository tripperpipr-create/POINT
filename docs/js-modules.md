...
- Новая ветка нажатия — в `*-actions.js` своего семейства, не в общий
  обработчик `main.js`.
- Новая группа сообщений хоста — свой контроллер плюс строка в
  `scripts/check-release-contracts.mjs` (контракт границ), в список копируемых
  файлов `distribution/apply-overlay.mjs` и в цепочку `node --check`.
  Пропустить второй список — получить «Cannot find module» в собранной IDE,
  которого не покажет ни один тест.
- Новый общий помощник — в существующий дом (`html-escape.js`,
  `format-units.js`, `view-runtime.js`), а не рядом с местом вызова.

## Чем это проверяется

`scripts/check-webview-exports.mjs` сверяет имена на стыке модулей вебвью:
опечатка в импорте не ломает сборку, esbuild подставит `undefined`.
