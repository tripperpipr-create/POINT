...
func (s *SQLite) SaveRun(ctx context.Context, r domain.Run) error {
	...
	_, err := s.db.ExecContext(ctx, `INSERT INTO runs(id,agent_id,profile_id,workspace_id,task,context_items,configuration_snapshot,provider,model,status,step,request_count,tools_used,changed_files,error,result,started_at,finished_at,duration_ms,controller_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
