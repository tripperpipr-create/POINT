...
// ── Реплика во время хода встаёт в очередь ────────────────────────────────
//
// Поле во время хода открыто, и мысль, пришедшая, пока модель думает, ставится
// в очередь: не уходит сразу (ход ещё идёт), не показывается в разговоре как
// сказанное (её ещё не сказали) и не теряется — уходит сама, когда ход
// кончится. Отправленное при этом живёт отдельно от поля: пока обе роли играл
// один masterDraft, набранное во время хода становилось «своей репликой».
{
  const ui = open()
  ...
  check('вторая мысль не ушла в ядро на наборе',
    sent(ui).length === 1,
    `во время хода ушло ${sent(ui).length} реплик вместо одной`)
  ui.click({ action: 'master-send' })
  const queued = ui.root.innerHTML
  check('отправка во время хода ставит реплику в очередь, а не в ядро',
    sent(ui).length === 1 && /class="hall-outbox-text"[^>]*>И заодно посмотри логи</.test(queued),
    'нажатие во время хода либо отправило вторую реплику, либо промолчало')
  check('реплика в очереди не показана как сказанная',
    !/<p>И заодно посмотри логи<\/p>/.test(queued),
    'реплика из очереди стоит в разговоре так, будто её уже отправили')
  check('поле освободилось для следующей мысли',
    !/<textarea[^>]*id="master-input"[^>]*>И заодно посмотри логи/.test(queued),
    'текст остался и в поле, и в очереди — уйдёт дважды')
  ...
  check('очередь ушла сама, когда ход кончился',
    sent(ui).length === 2 && sent(ui)[1].message === 'И заодно посмотри логи',
    `после конца хода отправлено ${sent(ui).length}: ${JSON.stringify(sent(ui).map(item => item.message))}`)
  check('ушедшая реплика покинула очередь',
    !/class="hall-outbox-text"/.test(ui.root.innerHTML),
    'реплика и ушла, и осталась в очереди')
}

// ── Очередь ждёт, если Мастер задал уточнения ─────────────────────────────
{
  const ui = open()
  ui.type('Собери отряд')
  ui.click({ action: 'master-send' })
  ui.type('И проверь бюджет')
  ui.click({ action: 'master-send' })
  ui.listeners['window:message']({ data: { type: 'master', turnFinished: true, master: {
    configured: true, config: { model: 'qwen' },
    history: [
      { id: 'u1', role: 'user', content: 'Собери отряд' },
      { id: 'a1', role: 'assistant', content: 'Уточню.', questions: ['Какой бюджет?'] },
    ],
  } } })
  check('после уточнений очередь не уходит сама',
    sent(ui).length === 1,
    'реплика из очереди ушла как ответ на вопросы, которых человек не видел, когда её писал')
  check('пауза названа, отправка под рукой',
    ui.root.innerHTML.includes('Мастер задал уточнения') && ui.root.innerHTML.includes('data-action="master-queue-send"'),
    'очередь молча стоит — непонятно, почему она не ушла')
}
