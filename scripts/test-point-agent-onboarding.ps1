...
if (-not $Screenshot) { $Screenshot = Join-Path $buildRoot 'code-oss\point-agent-onboarding.png' }
$listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 0)
$listener.Start(); $port = ([System.Net.IPEndPoint]$listener.LocalEndpoint).Port; $listener.Stop()
$testRoot = Join-Path $buildRoot ("agent-onboarding-$([guid]::NewGuid().ToString('N'))")
...
try {
  ...
  $result = & node (Join-Path $PSScriptRoot 'verify-point-agent-onboarding.mjs') "http://127.0.0.1:$port" ([System.IO.Path]::GetFullPath($Screenshot))
  if ($LASTEXITCODE -ne 0) { throw 'Point Agent onboarding verification failed.' }
  $result
} finally {
  if ($process) {
