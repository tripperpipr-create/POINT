...
export function createAgentWorkflowEditors(dependencies) {
  ...
  function profileEditor() {
    ...
    const identityStep = `<section class="profile-config-block profile-step" data-step-panel="identity"><header><span>01</span><div><strong>Личность класса</strong><small>Имя, роль и правила класса — с чем персонаж выйдет из найма</small></div></header>
        <p class="create-step-why">Имя класса видно в списке найма; нанятый персонаж получит его как стартовое. Промпт задаёт стиль рассуждения, правила — жёсткие ограничения на каждый квест.</p>
        <label>Персонаж<select id="settings-profile" ${creating?'disabled':''}>${creating?'<option>Новый класс</option>':''}${profiles.map(item=>`<option value="${esc(item.id)}" ${item.id===profile.id?'selected':''}>${esc(item.name)} · ${esc(agentClass(item))}</option>`).join('')}</select></label>
        <label>Имя<input id="profile-name" maxlength="200" value="${esc(profile.name)}" placeholder="Например, SAGE-7" required></label>
        <label>Класс и особенности<textarea id="role-description" rows="3" maxlength="4096" placeholder="В чём персонаж эксперт и какой результат от него нужен">${esc(profile.roleDescription)}</textarea></label>
    ...
    const toolsStep = `<section class="tool-section profile-config-block profile-step" data-step-panel="tools"><header><span>03</span><div><strong>Умения и разрешения</strong><small>Пресет — быстрый старт; бейдж «доказательство» помечает верификатор</small></div><b>${enabled.size} / ${tools.length}</b></header>
        <p class="create-step-why">Если разрешены правки файлов, включите умение с доказательством — иначе квест с критериями готовности не стартует.</p>
        ...
        <div class="permission-legend"><span><i class="safe"></i>Чтение и локальный индекс — автоматически</span><span><i class="review"></i>Изменение — только через diff</span><span><i class="approval"></i>Команды и свои умения — с подтверждением</span><span><i class="verify"></i>Доказательство готовности — успешный тест/сборка</span></div>
    ...
    const limitsStep = `<section class="profile-config-block profile-step" data-step-panel="limits"><header><span>04</span><div><strong>Лимиты и подтверждения</strong><small>Рекомендуемый лимит ходов для проверяемых квестов — 30</small></div></header>
        ...
        <label>Политика подтверждений<select id="approval-mode"><option value="safe" ${profile.approvalMode==='safe'?'selected':''}>Опасные действия: diff и команды</option><option value="always" ${profile.approvalMode==='always'?'selected':''}>Подтверждать каждое умение</option></select></label>
        ${creating ? hireSummaryPanel(profile, readiness) : ''}
        ${!creating?`<div class="profile-danger-row"><button type="button" class="secondary" data-action="export-profile">Экспорт класса</button></div>`:''}
      </section>`
    ...
    return shell(`<main class="settings agent-studio profile-wizard ${creating ? 'create-flow' : ''}">
      <div class="section-title"><span>${creating?'НАЙМ ПЕРСОНАЖА':'КАРТОЧКА ПЕРСОНАЖА'}</span><em>${countOf(profiles.length, 'агент', 'агента', 'агентов')}</em></div>
      ${isWide?'<button class="secondary roster-back" type="button" data-action="close-profile-editor">← К ростеру</button>':'<button class="secondary roster-expand" type="button" data-action="open-roster">Развернуть ростер →</button>'}
      ${creating ? '' : `<div class="studio-actions"><button class="secondary" data-action="new-profile">＋ Нанять</button><button class="secondary" data-action="duplicate-profile">⧉ Двойник</button><button class="secondary" data-action="import-profile">⇧ Призвать</button></div>`}
      ${creating ? `<header class="create-flow-hero"><div><span class="quest-label">ГИЛЬДИЯ / ИНТЕРАКТИВНЫЙ НАЙМ</span><h1>Соберите агента по шагам</h1><p>Класс даёт роль и умения. Дальше — личность, проверка модели, разрешения и лимиты. В конце — сводка и старт квеста.</p></div>${step === 'class' ? hireLiveChecklist(profile) : ''}</header>` : `${agentCharacterCard(profile)}${readinessBanner(profile, { showFixes: true })}`}
  ...
  function toolBuilder() {
    ...
    return shell(`<main class="settings tool-builder tool-arsenal"><header class="hub-page-head"><div><span>АРСЕНАЛ</span><h1>Свои инструменты</h1><p>Пять стадий ниже: от шаблона до теста. После сохранения включите инструмент в список разрешённых у агента.</p></div><em>${customTools.length}</em></header>
  ...
  function workflowBuilder() {
    ...
    return shell(`<main class="settings workflow-builder hybrid-flows">
      <div class="section-title"><span>КАМПАНИИ / ФЛОУ</span><em>${workflows.length} flow</em></div>
