...
export function createHallOnboardingViews(dependencies) {
  ...
  // Заголовок шапки — название раздела. Где внутри раздела человек находится,
  // показывает строка вкладок под шапкой, поэтому крошка больше не повторяет
  // путь «ПРОЕКТ · мир / РАЗДЕЛ / ПОДРАЗДЕЛ»: имя мира стоит в рейке, подраздел —
  // выделенной вкладкой. Подписи берутся из HALL_SECTIONS, второго списка нет.
  function hallCrumb() {
    const section = HALL_SECTIONS.find(item => item.tabs.includes(ui.state.selectedTab))
    if (section) return section.label
    // Отдельные окна IDE — не разделы Чертога, но назвать экран всё равно надо.
    // Незнакомая вкладка называется настройками проекта, а не Гильдией: запасной
    // вариант, указывающий в конкретное неверное место, врёт увереннее общего.
    return { statistics: 'Статистика', docker: 'Контейнеры' }[ui.state.selectedTab] || 'Настройки проекта'
  }
  ...
  function hallSectionBadge(id, pendingSets, waiting) {
    // Ноль не печатается: пустое место и есть «ничего не ждёт». Точка-заглушка
    // стояла у каждого раздела рейки и читалась как пятая колонка значков.
  ...
  function workspaceTrustRequired() {
    return shell(`<main class="workspace-locked">
      ...
      <h2>Инструменты агентов пока недоступны</h2>
      <p>Point не запускает локальное ядро, команды и инструменты без вашего разрешения для этого мира.</p>
  ...
  function offline() {
    ...
    if (errored) {
      return shell(`<main class="offline"><div class="orbit"><span>!</span></div><span class="quest-label">Ядро Point</span><h2>Ядро не поднялось</h2><p>${esc(detail || 'Локальный процесс завершился с ошибкой. Откройте журнал ядра — там полный лог, а не только «fetch failed».')}</p><div class="empty-next"><button class="primary" data-action="start-server">Повторить запуск</button><button class="secondary" data-action="show-output">Журнал ядра</button></div><small>Антивирус или брандмауэр иногда блокируют порт 127.0.0.1</small></main>`)
    }
    return shell(`<main class="offline"><div class="orbit"><span>✦</span></div><span class="quest-label">Гильдия Point</span><h2>${starting?'Запускаем локальное ядро…':'Ядро остановлено'}</h2><p>Локальное ядро подключает выбранного агента, модель и безопасные инструменты только для текущего проекта.</p><button class="primary" data-action="start-server" ${starting?'disabled':''}>${starting?'Подключаемся…':'Запустить ядро'}</button><small>Журнал и файлы остаются на вашем компьютере</small></main>`)
  }
  ...
  function firstAgentProposal() {
    ...
    else reasons.push('Индекс ещё не построен — предлагаю универсальный шаблон, его можно сменить.')
    if (template) reasons.push(`Роль «${template.name}» даёт ${countOf((template.allowedTools || []).length, 'инструмент', 'инструмента', 'инструментов')} без скрытого расширения прав.`)
  ...
  function onboarding() {
      ...
      panel = `<section class="onboarding-panel onboarding-complete-panel"><h2>Гильдия готова</h2><p>Центр командования — квест, отряд, компаньон и активные запуски на одном экране. Компаньон по-прежнему только рекомендует.</p><ul class="onboarding-facts"><li><i>✓</i> Компаньон: ${esc(ui.onboardingDraft.companionPreset || 'balanced')}</li><li><i>✓</i> Распоряжение: ${esc(ui.onboardingDraft.orchestratorPreset || boot.orchestrator?.preset || 'conductor')}</li><li><i>${agents.length ? '✓' : '·'}</i> Агентов: ${agents.length}</li><li><i>${connections.length ? '✓' : '·'}</i> Подключений: ${connections.length}</li><li><i>${indexReady ? '✓' : '·'}</i> Индекс: ${indexReady ? 'готов' : 'можно позже'}</li></ul><div class="onboarding-next-actions"><button type="button" class="primary" data-action="complete-onboarding">Открыть обзор гильдии</button><button type="button" class="secondary" data-action="tab" data-tab="quests">Поставить первый квест</button></div>
        <div class="onboarding-later">
          ...
          <div>
            ...
            <button type="button" class="secondary" data-action="tab" data-tab="agents">Инструменты и разрешения агента</button>
          </div>
          <small>Первый агент уже получил навыки и инструменты основного профиля — это дальнейшее развитие, а не условие запуска.</small>
    ...
    return shell(`<main class="onboarding onboarding-wizard">
      <header class="onboarding-hero"><div><span>Первый запуск</span><h1>Point Agent Hub</h1><p>${esc(heroCopy)}</p></div></header>
  ...
  function agentConstructor() {
    ...
    const toolsPanel = `<section class="tool-section profile-config-block profile-step" data-step-panel="tools"><header><span>07</span><div><strong>Инструменты</strong><small>Список разрешённых инструментов и правило вызова</small></div><b>${enabled.size} / ${tools.length}</b></header>${companionSuggestionChipsHtml('tools', draft)}<div class="permission-presets">${TOOL_PRESETS.map(item => `<button type="button" class="${activeToolPresetId(draft) === item.id ? 'on' : ''}" data-action="constructor-tool-preset" data-preset="${esc(item.id)}"><b>${esc(item.label)}</b><small>${esc(item.hint)}</small></button>`).join('')}</div><div class="tool-policy-legend"><span>Опасность — насколько рискован инструмент</span><span>Правило — разрешить, спросить или запретить</span><span>Запрет действует без подтверждения</span></div><div class="tool-policy-grid">${tools.map(tool => {
    ...
    const reviewActions = step === 'review'
      ? (draft.status === 'draft'
        ? `<button type="button" class="secondary" data-action="save-constructor">Сохранить черновик</button><button type="button" class="primary" data-action="activate-agent-draft">Сохранить и активировать</button><button type="button" class="danger-button" data-action="reject-agent-draft">Отклонить</button>`
  ...
  function conversation() {
    ...
    if (!details) {
      ...
      return `<div class="empty quest-board"><h3>Новый квест ждёт брифинга</h3><p>Заполните задачу, цель и критерии. Предпросмотр обязателен перед стартом — покажет инструменты, подтверждения и условия завершения без вызова модели.</p><div class="examples"><button data-example="Объясни архитектуру проекта и основные точки входа." data-goal="Понятная карта системы для нового разработчика" data-criteria="Перечислены точки входа и ключевые пакеты&#10;Выводы опираются на конкретные файлы" data-constraints="Не менять файлы&#10;Не запускать команды"><span>Изучить</span>Объяснить архитектуру</button><button data-example="Найди потенциальную ошибку и предложи минимальное исправление." data-goal="Минимальный diff, который устраняет дефект" data-criteria="Указаны файл и сценарий отказа&#10;После правки проходит узкий тест или объясняется, почему тест невозможен" data-constraints="Не трогать нерелевантные модули"><span>Починить</span>Найти ошибку</button><button data-example="Запусти тесты проекта и объясни результат." data-goal="Фактический результат проверки, а не предположение" data-criteria="Выполнена распознаваемая команда теста/сборки&#10;Exit code и краткий разбор зафиксированы" data-constraints="Не менять код без отдельного подтверждения"><span>Проверить</span>Проверить тесты</button></div>${hasProfiles?'':`<button type="button" class="primary empty-cta" data-action="tab" data-tab="agents">Сначала создайте агента →</button>`}</div>`
    }
    const attached = details.run.contextItems || []
    const head = `<article class="message user"><small>Квест</small><p>${esc(details.run.task)}</p>${attached.length?`<div class="run-context">${attached.map(item=>`<span title="${esc(item.path || item.label)}">${item.kind==='workspace_file'?'▤':'¶'} ${esc(item.label)}${item.truncated?' · сокращено':''}</span>`).join('')}</div>`:''}</article>`
  ...
  function agentRunPreviewMarkup() {
    ...
    if(!ui.agentRunPreview){
      if(!ui.taskDraft.trim()) return '<section class="agent-preflight idle"><strong>Предпросмотр перед стартом</strong><span>Заполните брифинг и нажмите «Предпросмотр» — увидите точный промпт, инструменты, подтверждения и условия завершения без вызова модели.</span></section>'
      return '<section class="agent-preflight idle nudge"><strong>Брифинг готов к предпросмотру</strong><span>Проверьте запуск перед «Принять квест»: точный промпт, инструменты и доказательства готовности. Без предпросмотра старт заблокирован.</span><button type="button" class="secondary" data-action="preview-run">Запустить предпросмотр</button></section>'
    }
  ...
  function composerActionsHtml(profile, active, cursor) {
    ...
    if (!profile) return '<button type="button" class="send" disabled><span>Сначала создайте агента</span></button>'
    ...
    return `<button type="button" class="preflight-button" data-action="preview-run" title="Показать точную инструкцию, инструменты и токены"><span aria-hidden="true">✓</span> Предпросмотр</button><button type="submit" class="send" ${completionBlocked?'disabled':''} title="${completionBlocked?'Сначала разрешите запуск команд или измените критерии':'Начать квест'}"><span>${completionBlocked?'Исправьте разрешения':'Принять квест'}</span><span aria-hidden="true">→</span></button>`
  }
  ...
  function chat() {
    ...
    const readinessTip = !active && profile && !readiness.ready
      ? `<aside class="readiness-banner compact"><span>!</span><div><strong>Старт заблокирован · агент не готов</strong><small>${esc(readiness.issues[0])}</small></div><button type="button" class="secondary" data-action="fix-profile-step" data-step="${esc(firstUnreadinessStep(profile))}">Исправить →</button></aside>`
      : ''
    const briefTip = questBriefGuidanceHtml(active, cursor)
    const emptyRoster = !profiles.length
      ? `<aside class="readiness-banner"><span>✦</span><div><strong>Сначала создайте агента</strong><small>Без карточки квест не стартует. Выберите шаблон — модель и инструменты подставятся.</small></div><button type="button" class="primary" data-action="tab" data-tab="agents">Создать агента →</button></aside>`
      : ''
  ...
  function history() {
    ...
    return shell(`<main class="history"><div class="section-title"><span>История квестов</span><em>${runs.length}</em></div><p class="history-intro">Прошлые запуски. Полоса качества показывает доказательства, сбои инструментов и отказы — откройте запуск, чтобы увидеть подробности.</p>${runComparison(runs, diagnosticsByRun)}${visibleRuns.length?visibleRuns.map(run=>{const diagnostics=diagnosticsByRun.get(run.id);const profile=profiles.get(run.profileId)||run.configurationSnapshot?.profile;return `<button class="run quest-row" data-action="load-run" data-id="${esc(run.id)}"><div>${status(run.status)}<small>${formatDateTime(run.startedAt)}</small></div><strong>${esc(run.task)}</strong><footer><span>${esc(profile?.name || run.profileId || 'агент')} · ${esc(run.model)}</span><span>ходов: ${run.step}${diagnostics?.approvals?.pending?` · ждёт решений: ${diagnostics.approvals.pending}`:''}</span></footer>${diagnostics?`<div class="run-diagnostic-strip health-${esc(diagnostics.health)}"><span>${esc(healthLabels[diagnostics.health]||diagnostics.health)}</span><span>${formatDuration(diagnostics.durationMs)}</span><span>${diagnostics.model?.usageReported?`${esc(diagnostics.model.totalTokens||0)} ток.`:'токены —'}</span>${historyQualitySignals(diagnostics)}</div>`:''}</button>`}).join(''):`<div class="empty compact"><span class="quest-label">История проекта</span><h3>Здесь пока нет глав</h3><p>Завершённые и прерванные квесты попадут в историю проекта. Начните с вкладки «Квесты».</p><button class="primary" data-action="tab" data-tab="quests">К брифингу →</button></div>`}${runs.length>visibleRuns.length?`<p class="history-limit">Показаны 100 последних глав из ${runs.length}. Полная история хранится локально.</p>`:''}</main>`)
  }
  ...
  function guildRoster() {
    ...
    if (!profiles.length) {
      ...
      const primaryHire = firstTemplate
        ? `<button class="primary" data-action="use-template" data-template="${esc(ui.hirePreviewTemplateId || firstTemplate.id)}">Добавить агента →</button>`
      ...
      return shell(`<main class="guild-roster empty-roster create-flow">
        <header class="guild-heading"><div><h1>Добавьте первого агента</h1><p>Выберите переносимый основной профиль. Point создаст для него адаптацию текущего проекта, не дублируя самого агента.</p></div></header>
        
        ${templatePickerHtml(templates, { interactive: true })}
        <div class="empty-next">${primaryHire}<button class="secondary" data-action="open-agent-constructor">Настроить профиль</button><button class="secondary" data-action="new-profile">Новый агент</button><button class="secondary" data-action="import-profile">Импортировать профиль</button></div>
      </main>`)
    }
    ...
    const synergyHtml = synergies.length
      ? `<section class="roster-synergy"><header><strong>Покрытие команды</strong><span>${synergies.length}</span></header><div>${synergies.map(item => `<button type="button" data-action="select-roster-profile" data-id="${esc(item.id)}"><div><strong>${esc(item.name)}</strong><small>${esc(item.className)} · ${countOf(item.shared, 'общий инструмент', 'общих инструмента', 'общих инструментов')}</small></div><em>${item.score}%</em></button>`).join('')}</div><p>Пересечение разрешённых инструментов помогает видеть дублирование возможностей.</p></section>`
      : `<section class="roster-synergy empty"><header><strong>Покрытие команды</strong><span>0</span></header><p>Нет пересечения инструментов с другими агентами — расширьте навыки текущей команды, прежде чем добавлять новую роль.</p></section>`
    ...
    const blueprintsHtml = blueprints.length
      ? `<details class="hire-drawer"><summary>Блюпринты для всех проектов <span>${blueprints.length}</span></summary><p>Это образцы, общие для всех миров. Правка блюпринта видна каждому проекту, где есть его агент; сам агент — копия и живёт только в своём мире.</p>${templatePickerHtml(blueprints, { compact: true })}</details>`
      : ''
    // Карточка агента — RPG-лист в три строки: уровень рядом с именем, опыт до
    // следующего уровня и две характеристики. Эмблемы в рамке нет: агента
    // отличает имя и роль, а рамка со штриховкой была тем же знаком у каждого.
    const rosterCard = profile => {
      const stats = agentProgress(profile)
      const reliability = stats.reliability === undefined ? '—' : `${stats.reliability}%`
      const ready = profileReadiness(profile).ready
      const activeRun = (ui.state.boot?.runs || []).find(run => run.profileId === profile.id && runIsLive(run))
      const statusClass = activeRun ? 'channeling' : ready ? 'idle' : 'setup'
      const statusLabel = activeRun ? 'В работе' : ready ? 'Свободен' : 'Нужна настройка'
      const catalogSize = (ui.state.boot?.toolCatalog || []).length
      const allowedCount = (profile.allowedTools || []).length
      const toolShare = Math.min(100, Math.round((allowedCount / Math.max(1, catalogSize || 8)) * 100))
      const toolsLabel = catalogSize ? `${allowedCount}/${catalogSize}` : String(allowedCount)
      return `<button class="roster-card ${profile.id === selected.id ? 'selected' : ''} ${ready ? '' : 'needs-setup'}" data-action="select-roster-profile" data-id="${esc(profile.id)}"${profile.id === selected.id ? ' aria-current="true"' : ''}><header class="roster-card-head"><div><strong>${esc(profile.name)}</strong><small>${esc(agentClass(profile))}</small></div><em class="roster-lv">Ур ${stats.level}</em></header><div class="roster-bars"><label><span>Опыт</span><b>${stats.xp}/${stats.xpTarget}</b></label><progress class="xp" value="${stats.xp}" max="${stats.xpTarget}"></progress><label><span>Надёжность</span><b>${reliability}</b></label><progress class="vital" value="${stats.reliability || 0}" max="100"></progress><label><span>Инструменты</span><b>${toolsLabel}</b></label><progress class="mana" value="${toolShare}" max="100"></progress></div><footer><span class="roster-status ${statusClass}"><i></i>${statusLabel}</span></footer></button>`
    }
    const toLevel = progress.xpTarget - progress.xp
    return shell(`<main class="guild-roster">
      <header class="guild-heading"><div><h1>Команда текущего проекта</h1><p>${countOf(profiles.length, 'агент', 'агента', 'агентов')} · ${readyCount} ${plural(readyCount, 'готов', 'готовы', 'готовы')} к работе. Улучшайте агентов навыками, инструментами и правами вместо создания дублей.</p></div><div><button class="secondary" data-action="new-profile" type="button">Добавить агента</button></div></header>
      <div class="guild-summary"><span><small>Квестов</small><b>${runs.length}</b></span><span><small>Завершено</small><b>${completedRuns}</b></span><span><small>Надёжность отряда</small><b>${guildReliability===undefined?'—':`${guildReliability}%`}</b></span><p>Метрики рассчитаны локально по сохранённым запускам Point.</p></div>
      <div class="guild-layout">
        <section class="roster-list" aria-label="Агенты"><div>${profiles.map(rosterCard).join('')}</div></section>
        <section class="roster-detail"><header><div><h2>${esc(selected.name)}</h2><p>${esc(agentClass(selected))}</p></div><em class="roster-lv">Ур ${progress.level}</em></header><div class="level-track"><label><span>Опыт</span><b>${progress.xp} / ${progress.xpTarget}</b></label><progress class="xp" value="${progress.xp}" max="${progress.xpTarget}"></progress><small>До уровня ${progress.level + 1} — ещё ${countOf(toLevel, 'подтверждённый квест', 'подтверждённых квеста', 'подтверждённых квестов')}.</small></div>${/* Готовому агенту баннер «готов к квесту» повторял кнопку «Поставить задачу» строкой ниже. */''}${readiness.ready ? '' : readinessBanner(selected, { editAction: 'edit-roster-profile' })}${activeQuestHtml}<footer class="roster-actions"><button class="primary" data-action="start-roster-quest">${readiness.ready?'Поставить задачу':'Настроить и к задаче'}</button><button class="secondary" data-action="edit-roster-profile">Проектная адаптация</button><button class="secondary" data-action="open-agent-constructor-edit">Улучшить профиль</button>${hubModeAvailable()?`<button class="danger-button" data-action="disband-agent" data-id="${esc(selected.id)}">Распустить</button>`:''}</footer><div class="detail-stats"><span><small>Задачи</small><b>${progress.quests}</b></span><span><small>Завершено</small><b>${progress.completed}</b></span><span><small>Надёжность</small><b>${reliability}</b></span><span><small>Удачных вызовов</small><b>${progress.toolMastery===undefined?'—':`${progress.toolMastery}%`}</b></span></div><section class="character-purpose"><small>Роль</small><p>${esc(selected.roleDescription || 'Описание роли пока не задано.')}</p>${(selected.goals||[]).length?`<ul>${selected.goals.map(goal=>`<li>${esc(goal)}</li>`).join('')}</ul>`:''}</section><section class="equipped-tools"><header><strong>Навыки и инструменты</strong><span>${tools.length}</span></header><div>${tools.map(tool=>`<span>${esc(tool)}</span>`).join('')||'<em>Инструменты не выбраны</em>'}</div></section>${persistentProfileSummary(selected)}<section class="agent-runtime"><div><small>Модель</small><strong>${esc(selected.model)}</strong></div><div><small>Провайдер</small><strong>${esc(providerPreset(selected)?.name||selected.provider)}</strong></div><div><small>Подтверждения</small><strong>${selected.approvalMode==='always'?'каждое действие':'опасные действия'}</strong></div><div><small>Доказательства</small><strong>${readiness.hasVerifier?'есть верификатор':'нет верификатора'}</strong></div></section>${synergyHtml}<div class="recent-quests"><header><strong>История</strong><span>${selectedRuns.length}</span></header>${selectedRuns.map(run=>`<button data-action="load-run" data-id="${esc(run.id)}"><div><strong>${esc(run.task)}</strong><small>${formatDateTime(run.startedAt)} · ходов ${run.step}</small></div>${status(run.status)}</button>`).join('')||'<p>У агента ещё нет завершённых квестов.</p>'}</div></section>
      </div>
      ${templates.length ? `<details class="hire-drawer"><summary>Основные профили команды <span>${templates.length}</span></summary>${templatePickerHtml(templates, { compact: true })}</details>` : ''}
      ${blueprintsHtml}
      ${/* Дверь к схемам проекта. Роспуск агента отказывает словами
           «замените его в схеме», а схемы Мастер создаёт сам под каждый
           квест и они его переживают. Экран уборки существовал
           (projectFlowsView), но входа к нему не было ни одного: кнопки
           data-tab="flows" нет нигде, кроме карточек внимания при
           конфликте веток. Отказ называл дверь, которой не существовало.
           Показываем только когда схемы есть — пустой раздел не зовут. */''}
      ${(ui.state.boot?.flows || []).length ? `<p class="hall-fineprint roster-flows-note">Агента, занятого в схеме, распускают в разделе <button type="button" class="linkish" data-action="tab" data-tab="flows">Схемы</button>.</p>` : ''}
    </main>`)
  }
