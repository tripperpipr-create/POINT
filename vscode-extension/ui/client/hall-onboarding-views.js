...
export function createHallOnboardingViews(dependencies) {
  ...
  function guildRoster() {
    ...
    return shell(`<main class="guild-roster">
      ...
      ${/* Дверь к схемам проекта. Роспуск персонажа отказывает словами
      ...
      ${(ui.state.boot?.flows || []).length ? `<nav class="hub-transitional" aria-label="Смежные разделы">
        <b>ПЕРЕЙТИ</b>
        <button type="button" class="hall-btn is-sm" data-action="tab" data-tab="flows">СХЕМЫ ПРОЕКТА</button>
      </nav>` : ''}
    </main>`)
  }
