...
function currentDecision() {
  ...
  return items.find(item => item.id === decisionPick) || items[0]
}
...
function sendDecisionResolve(id, intent) {
  ...
  if (decisionsStatus === 'loading') return false
  const proposal = taskProposalById(id)
  if (intent.path === '/api/quest-proposals/decide' && intent.value === 'start' && proposal?.brief) {
    if (proposalEditId === id || proposalStarting.has(id) || proposalModifying.has(id) || !['ready', 'approved'].includes(proposal.brief.state) || proposal.brief.openQuestions?.length) return false
...
function decisionDetailHtml(item) {
  if (!item) {
    ...
    if (decisionsStatus === 'error') {
      ...
      <p class="hall-note">Запрос к ядру не удался: ${esc(decisionsError)}. Здесь не пусто — здесь неизвестно. Нажмите «Повторить», чтобы спросить ядро снова.</p></div>`
    }
    ...
    if (decisionsStatus === 'loading' || decisionsStatus === 'idle') {
  ...
  return shell(`<div class="hall-split">
    <div class="hall-queue">
      <header>
        ...
        <small>${decisionsStatus === 'loading' ? 'загрузка…' : decisionsStatus === 'error' ? 'не удалось' : 'по времени ожидания'}</small>
      </header>
      ${decisionsStatus === 'error' ? `<div class="hall-strip">
        <span>Очередь не обновилась: ${esc(decisionsError)}</span>
...
function companionBubbleHtml(item, messageIndex = -1) {
  ...
  const blindSpotHtml = role === 'assistant' && caveats.length
    ...
    : ''
  const mark = companionFeedbackMarks.get(String(item.id || ''))
  const markHtml = [
...
function companionStreamingBubbleHtml() {
  ...
  if (companionStreamReply) {
    return companionBubbleHtml({ role: 'assistant', content: companionStreamReply, mode: 'streaming', streaming: true })
  }
...
function companionThreadHtml() {
  const bubbles = companionMessages.map((item, index) => item?.mode === 'streaming' ? '' : companionBubbleHtml(item, index)).join('')
...
function patchCompanionStreamingBubble() {
  const thread = root.querySelector('#companion-thread')
  if (!thread || !companionLoading) return
  const follow = companionAutoFollow || threadNearBottom(thread)
...
function companionSpeakInterventions() {
  return (state.boot?.companionInterventions || []).filter(item => item?.level === 'critical' || item?.level === 'warning')
}
function companionNudgeBannerHtml() {
  const item = companionSpeakInterventions()[0]
  if (!item) return ''
  const probeVisible = companionInterventionProbe?.interventionId === item.id && companionInterventionProbe?.connectionId === item.relatedId
...
function decisionHotkey(event) {
  if (state.selectedTab !== 'decisions') return ''
