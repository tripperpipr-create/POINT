...
export function masterComposeMetaHtml (loading) {
  ...
  //
  // Короткой строкой: длинная («реплика уйдёт после ответа») занимала полряда
  // рядом с «Остановить» и повторяла подпись очереди над полем, где каждая
  // строка и так говорит «уйдёт после ответа Мастера».
  return loading
    ? 'Enter — в очередь'
