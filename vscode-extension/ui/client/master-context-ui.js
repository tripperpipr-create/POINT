...
export function masterContextAddHtml(id, esc, sending) {
  // Две двери, а не одна. Самый частый контекст — то, что открыто в редакторе
  // прямо сейчас, и путь к нему был длиннее всех: выбор источника, потом диалог
  // файлов. Обработчик «взять открытый файл или выделение» написан в расширении
  // целиком (master-chat-controller.js) и до сих пор не был нарисован ничем —
  // действие master-context-attach не встречалось ни в одном шаблоне.
  //
  // В подсказке названы и «@», и перетаскивание: и то и другое работает, и об
  // этом не было сказано нигде.
  return `<button type="button" class="hall-context-add" data-action="master-context-attach" data-session="${esc(key(id))}" ${sending ? 'disabled' : ''} title="Взять открытый файл или выделение в редакторе">Открытый файл</button>
    <button type="button" class="hall-context-add" data-action="master-context-pick" data-session="${esc(key(id))}" ${sending ? 'disabled' : ''} title="Файлы, папки, ошибки, git diff и буфер терминала. То же открывает «@» в поле; файл можно перетащить мышью">@ Контекст</button>`
}
