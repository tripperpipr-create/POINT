...
function autonomousLearningHtml(stats) {
  ...
  const cards = items.slice(0, 12).map(item => {
    ...
    const memoryBlock = memory && item.memoryStatus
      ? `<aside class="agent-learning-memory is-${esc(item.memoryStatus)}"><header><span>Память агента</span><em>${esc(memoryStatusLabel[item.memoryStatus] || item.memoryStatus)}</em></header><p>${esc(memory.content || '')}</p><small>${esc(item.memoryKey || 'устойчивый принцип')} · независимых проектов: ${Number(item.memorySourceWorkspaces?.length || 1)}</small></aside>`
      : ''
